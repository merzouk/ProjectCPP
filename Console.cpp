...
{
       this->annuaire->load_annuaire_from_files(fileContactPrivate, fileContactPro);
}

void Console::rechercher_contact_nom()
{
      cout <<"rechercher_contact_nom" << endl;
      cout << "Veuillez saisir le nom du contact" << endl;
      string nom = get_str_value();
      char *ptr;
      ptr = str_to_char(nom, ptr);
      vector<Contact*> vects = this->annuaire->get_list_elts_by_last_name(to_upper_last_name(ptr));
...
void Console::rechercher_contact_code_postale()
{
      ...
      vector<Contact*> contacts = this->annuaire->get_list_elts_by_zip_code(zip_code);
...
void Console::rechercher_contact_departement()
{
      ...
      vector<Contact*> contacts = this->annuaire->get_list_elts_by_department(departement);
      this->annuaire->display(contacts);
}
...
void Console::supprimer_contact_pid()
{
      ...
      {
             cout << "Erreur inattendue pendant la tentative de suppression du contact par son id "<<pid << endl;
      }
...
void Console::modifier_contact_pid()
{
     ...
     {
            cout <<"Erreur inattendue s'est produite pendant la tentative de modification de l'adresse postale"<<endl;
     }
...
AdressePostale *Console::new_adress()
{
       ...
       cin.ignore();
       getline(cin, ville);
       AdressePostale * adr = new AdressePostale(numero, rue, complement, code_postale, ville);
       return adr;
}
...
void Console::ajouter_contact_prive()
{
       ...
       cin.ignore();
       getline(cin, sexe);
       cout <<"Veuillez saisir la situation Familliale (Marie, Celibataire, Veuf, Pasce ou autres : " << endl;;
       cin.ignore();
       ...
       ContactPrive *contactprive = new ContactPrive(dateNaissance, identifiant , n, p, sexe, situation, adressePostale);
...
void Console::ajouter_contact_professionnel()
{
      ...
      // }while((sexe != "F" || sexe != "M"));
       ...
       getline(cin, entreprise);
        //do{
              cout <<"Veuillez saisir le statut juridique valide de l'entreprise SARL/SA/SAS/EURL : " << endl;;
              cin.ignore();
              getline(cin, entreprise);
       ...
       p = str_to_char(prenom, p);
       int identifiant = get_next_pid();
       ContactProfessionel *contactpros = new ContactProfessionel(entr, statut,  email, identifiant , n, p, sexe, situation, adressePostale);
